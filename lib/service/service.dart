...
import 'package:upai_app/DTOs/auction_model/make_auction_post.dart';
import 'package:upai_app/DTOs/auction_model/submit_auction_post.dart';
...
// final responseData = json.decode();
  ...
  Future updateAuction(
      int productId, String buyerEmail, double suggestPrice) async {
    var uri = Uri(
      scheme: 'http',
      host: ip,
      port: 80,
      path: 'Auction/Unapply',
    );
    Map<String, dynamic> data = {
      'productId': productId,
      'buyerEmail': buyerEmail,
  ...
  Future makeAuction(MakingAuctionProduct argument) async {
    var uri = Uri(
      scheme: 'http',
      host: ip,
      port: 80,
      path: 'Auction/MakeAuction',
    );
    var json = jsonEncode(argument.toMap());
    var response = await client.post(
      uri,
  ...
  Future unmakeAuction(UnmakeAuction argument) async {
    var uri = Uri(
      scheme: 'http',
      host: ip,
      port: 80,
      path: 'Auction/UnmakeAuction',
    );
    // Map<String, dynamic> data = argument.toMap();
    var response = await client.delete(
      uri,
  ...
  Future submitAuction(SubmitAuctionProduct argument) async {
    var uri = Uri(
      scheme: 'http',
      host: ip,
      port: 80,
      path: 'Auction/SubmitAuction',
    );
    var response = await client.post(
      uri,
      body: argument.toJson(),
